...
public abstract class AbstractCasFilter extends AbstractConfigurationFilter {
	...
	public final void setServerName(String serverName) {
		String casServerName = PropertyConfigurer.getValue(CasPropertiesConfig.SERVER_NAME);
		serverName = casServerName + serverName;
		if ((serverName != null) && (serverName.endsWith("/"))) {
			this.serverName = serverName.substring(0, serverName.length() - 1);
			this.log.info(String.format("Eliminated extra slash from serverName [%s].  It is now [%s]",
							new Object[] { serverName, this.serverName }));
		} else {
			this.serverName = serverName;
		}
	}
